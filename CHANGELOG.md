...
- `VLLM_EXL3_NGRAM_TABLE=disk`. The packed n-gram table stays in the checkpoint: the loader keeps vLLM's memory-mapped safetensors views instead of copying into a resident int16 tensor, and each lookup gathers its unique rows on the host, uploads them, and decodes on the device. The table then costs page cache rather than 32 to 36 GiB of device memory. The host gather is a synchronization point, so this mode needs `--compilation-config '{"cudagraph_mode": "PIECEWISE", "splitting_ops": [...attention ops..., "vllm.exl3_ngram_lookup"]}'`; the loader refuses FULL graph modes with that message. Default stays `resident`. Tests: `tests/test_ngram_layouts.py`.
